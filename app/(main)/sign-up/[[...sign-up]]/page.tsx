...
export default function SignUpPage() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccess(true);
      setLoading(false);
      // توجيه المستخدم تلقائياً بعد ثانيتين للوحة التحكم
      setTimeout(() => {
    ...
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <div className="p-8 border rounded-xl shadow-sm bg-card text-center max-w-md w-full">
        ...
        {success ? (
          ...
          <form onSubmit={handleEmailSignUp} className="flex flex-col gap-4 text-right">
            <div>
        ...
        <p className="mt-6 text-sm text-muted-foreground">
          لديك حساب بالفعل؟ <Link href="/sign-in" className="text-primary hover:underline">تسجيل الدخول</Link>
        </p>
      </div>
    </div>
  );
